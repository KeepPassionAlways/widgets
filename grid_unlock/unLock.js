...
(function(exports, doc) {
	 ...
	 * @description 使用
	 * @constructor
	 * @param {HTMLElement} dom  canvas画布
	 ...
	 * innerRoundColor - 内院背景颜色
	 * @param {function} result in callback 回调函数
	 */
	function GridUnLock(dom, options, callback) {
		var self = this;
		...
		self.offsetTop = dom.offsetTop;

		var x = (self.cw - 2 * self.offsetX - self.r * 2 * 3) / 2;
		var y = (self.ch - 2 * self.offsetY - self.r * 2 * 3) / 2;
		
		self.pointArr = self.drawLocalPoint(x, y);
		// 初始化事件监听
		self.initListeners(dom, cxt, callback);

		self.draw(self.pointArr, [], null, cxt);
	}
	 ...
	 * @param {Number} y
	 */
	GridUnLock.prototype.drawLocalPoint = function(x, y) {
		var data = [],
			self = this,
	 ...
	 */
	GridUnLock.prototype.initListeners = function(canvasDom, cxt, callback) {
		var linePoint = [],
			that = this;

		// 触摸开始
		canvasDom.addEventListener('touchstart', function(e) {
			that.selectPoint(e.touches[0], linePoint);
		});
		...
		canvasDom.addEventListener('touchmove', function(e) {
			...
			that.selectPoint(touches, linePoint);

			cxt.clearRect(0, 0, that.cw, that.ch);

			that.draw(that.pointArr, linePoint, {
				x: touches.pageX,
		...
		canvasDom.addEventListener('touchend', function(e) {
			cxt.clearRect(0, 0, that.cw, that.ch);
			that.draw(that.pointArr, linePoint, null, cxt);
			// 暴露出来的函数
	 ...
	 */
	GridUnLock.prototype.selectPoint = function(touches, LinePoint) {
	 ...
	 * @param {Object} cxt canvas画笔
	 */
	GridUnLock.prototype.draw = function(pointArr, linePointArr, touchPoint, cxt) {
		var self = this,
			r = self.r,
			pI_2 = Math.PI * 2;
	 ...
	 * @param {Object} 默认配置
	 */
	GridUnLock.prototype.extend = function(options, defaultValue) {
		var self = this;
		options = options || {};
	...
	exports.getUnLock = function(dom, options, callback) {
		return new GridUnLock(dom, options, callback);
	};
	...
	window.GridUnLockTools = exports;
}({}, document));